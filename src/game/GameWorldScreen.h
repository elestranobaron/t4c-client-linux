...
class GameWorldScreen {
   public:
    ...
    /** Decode un lot de sprites ; retourne true quand le monde est pret. */
    bool PollLoad(int batchSize = 256);
   ...
   private:
    ...
    static bool isNpcUnit(std::int32_t unitId, std::uint16_t appearance);
    ...
    bool drawUnitSprite(SDL_Renderer *renderer, float screenX, float screenY, std::int32_t unitId,
                        std::uint16_t appearance, int direction, int animFrame, bool attacking) const;
