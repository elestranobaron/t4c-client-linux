...
//////////////////////////////////////////////////////////////////////////////////////////
namespace {
constexpr int kWireInt32Size = 4;
}
//////////////////////////////////////////////////////////////////////////////////////////
...
{	
	*i = 0;
	if( HEADER_SIZE + nPos + kWireInt32Size <= (int)vBuffer.size() ){
...
{
    *i = 0;
    if( HEADER_SIZE + nPos + (int)sizeof( short ) <= (int)vBuffer.size() ){
        *i =  vBuffer[ HEADER_SIZE + nPos++ ] << 8;
        *i += vBuffer[ HEADER_SIZE + nPos++ ];		
    }else{
        TFCPacketException *e = new TFCPacketException(1);
...
{
	*i = 0;
	if( HEADER_SIZE + nPos + (int)sizeof( char ) <= (int)vBuffer.size() ){
        *i = vBuffer[ HEADER_SIZE + nPos++ ];	
	}else{
        TFCPacketException *e = new TFCPacketException(1);
        throw(e);
...
//////////////////////////////////////////////////////////////////////////////////////////
{
	// TRUE quand la position de lecture a atteint (ou depasse) la fin des donnees.
	if( HEADER_SIZE + nPos >= (int)vBuffer.size() ){
		return TRUE;
	}else{
		return FALSE;
   }
